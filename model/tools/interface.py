...
import cv2
import numpy as np
from models.audio2pose import AudioToPoseLSTM 
...
def interpolate_pitch(y, y_flag):
    frame_num = y.shape[0]
    i = 0
    last = -1
    while i < frame_num:
...
def load_checkpoint(checkpoint_path, generator=None, kp_detector=None, ph2kp=None):
...
def get_image_pose(image_path):
    processor = config.OPENFACE_POSE_EXTRACTOR_PATH
    tmp_dir = "samples/tmp_dir"
    os.makedirs(tmp_dir, exist_ok=True)
    subprocess.call([processor, "-f", image_path, "-out_dir", tmp_dir, "-pose"])

    img_file = os.path.basename(image_path)[:-4] + ".csv"
    csv_file = os.path.join(tmp_dir, img_file)
    pos_data = pandas.read_csv(csv_file)
    i = 0
    pose = [pos_data[" pose_Rx"][i], pos_data[" pose_Ry"][i], pos_data[" pose_Rz"][i],
            pos_data[" pose_Tx"][i], pos_data[" pose_Ty"][i], pos_data[" pose_Tz"][i]]
...
def get_pose_from_audio(img, audio, audio2pose):
    ...
    generator = AudioToPoseLSTM().cuda().eval()
